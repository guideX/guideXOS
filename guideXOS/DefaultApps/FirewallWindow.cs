...
namespace guideXOS.DefaultApps {
    internal class FirewallWindow : Window {
        ...
        public FirewallWindow(int x,int y):base(x,y,520,360){ Title="guideXOS Firewall"; ShowInTaskbar=true; }
        public override void OnInput(){ base.OnInput(); if(!Visible) return; int mx=Control.MousePosition.X; int my=Control.MousePosition.Y; bool left=Control.MouseButtons.HasFlag(MouseButtons.Left);
            ...
            for(int i=0;i<modes.Length;i++){ bool over= mx>=bx && mx<=bx+bw && my>=by && my<=by+bh; if(left && over && !_clickLatch){ Firewall.Mode = modes[i]; _clickLatch=true; }
                bx+=bw+8; }
            if(!left) _clickLatch=false; }
        public override void OnDraw(){ base.OnDraw(); int cx=X+_pad; int cy=Y+_pad; int cw=Width-2*_pad; int ch=Height-2*_pad;
            // Header
