...
namespace guideXOS.DefaultApps {
    internal class Monitor : Window {
        ...
        public override void OnDraw() {
            ...
            int aX = 0;
            Render(ref aX, CPUUsage, (int)ThreadPool.CPUUsage);
            Render(ref aX, RAMUsage, (int)(Allocator.MemoryInUse * 100 / (Allocator.MemorySize == 0 ? 1 : Allocator.MemorySize)));
            DrawBorder();
        }
        private void Render(ref int aX, Chart chart, int pct) {
            WindowManager.font.DrawString(X + aX + chart.graphics.Width / 2 - WindowManager.font.MeasureString(chart.name) / 2, Y, chart.name + " " + pct.ToString() + "%");
            Framebuffer.Graphics.DrawImage(X + aX, Y + Height - chart.graphics.Height, chart.image, true);
            Framebuffer.Graphics.DrawRectangle(X + aX, Y, chart.graphics.Width, Height, 0xFF333333);
        ...
        private void DrawLineChart(int value, ref int lastValue, Chart chart, uint Color) {
            ...
            if (chart.writeX >= w) {
                chart.writeX = 0; // wrap: fade entire surface
