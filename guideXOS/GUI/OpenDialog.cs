...
namespace guideXOS.GUI {
    internal class OpenDialog : Window {
        ...
        public OpenDialog(int x, int y, int w, int h, string startPath, Action<string> onOpen) : base(x, y, w, h) {
            ...
            _selectedIndex = -1; _clickLock = false; _onOpen = onOpen;
        ...
        private void RefreshEntries() { if (_entries != null) { for (int i = 0; i < _entries.Count; i++) _entries[i].Dispose(); _entries.Clear(); } _entries = File.GetFiles(_currentPath); _selectedIndex = _entries.Count > 0 ? 0 : -1; }
        private void GoUp() { if (string.IsNullOrEmpty(_currentPath)) return; string path = _currentPath; if (path.Length > 0 && path[path.Length - 1] == '/') path = path.Substring(0, path.Length - 1); int last = path.LastIndexOf('/'); _currentPath = last >= 0 ? path.Substring(0, last + 1) : ""; RefreshEntries(); }
        private void OpenSelected() { if (_selectedIndex < 0 || _selectedIndex >= _entries.Count) return; var e = _entries[_selectedIndex]; if (e.Attribute == FileAttribute.Directory) { _currentPath = _currentPath + e.Name + "/"; RefreshEntries(); } else { string path = _currentPath + e.Name; _onOpen?.Invoke(path); path.Dispose(); this.Visible = false; } }

        public override void OnInput() {
            base.OnInput(); bool left = Control.MouseButtons.HasFlag(MouseButtons.Left); int mx = Control.MousePosition.X; int my = Control.MousePosition.Y; int cx = X + _padding; int cy = Y + _padding + 28; int listX = cx; int listY = cy + 24; int listW = Width - _padding * 2; int listH = Height - _padding * 2 - 60; int upW = 60; int upH = 22; int upX = cx; int upY = cy; int openX = X + Width - _padding - _btnW; int openY = Y + Height - _padding - _btnH; int cancelX = openX - 8 - _btnW; int cancelY = openY;
            if (left) { if (!_clickLock) { if (mx >= upX && mx <= upX + upW && my >= upY && my <= upY + upH) { GoUp(); _clickLock = true; return; } if (mx >= openX && mx <= openX + _btnW && my >= openY && my <= openY + _btnH) { OpenSelected(); _clickLock = true; return; } if (mx >= cancelX && mx <= cancelX + _btnW && my >= cancelY && my <= cancelY + _btnH) { this.Visible = false; _clickLock = true; return; } if (mx >= listX && mx <= listX + listW && my >= listY && my <= listY + listH) { int idx = (my - listY) / _rowH; if (idx >= 0 && idx < _entries.Count) { _selectedIndex = idx; _clickLock = true; return; } } } } else { _clickLock = false; }
        }
