...
namespace guideXOS.GUI {
    ...
    internal static class WindowManager {
        ...
        struct PendingWindow { public int Type; public int X, Y, W, H; }
        ...
        public static void EnablePerfTracking() {
            if (_perfTrackingEnabled) return;
        ...
        /// </summary>
        public static void EnqueueDisplayOptions(int x, int y, int w, int h) { PendingWindow pw; pw.Type = 1; pw.X = x; pw.Y = y; pw.W = w; pw.H = h; _pending.Add(pw); }
        /// <summary>
        /// Flush pending window creations
        /// </summary>
        public static void FlushPendingCreates() { if (_pending.Count == 0) return; for (int i = 0; i < _pending.Count; i++) { var pw = _pending[i]; if (pw.Type == 1) _ = new DisplayOptions(pw.X, pw.Y, pw.W, pw.H); } _pending.Clear(); }

        /// <summary>
        /// Move to End
        /// </summary>
        /// <param name="window"></param>
        public static void MoveToEnd(Window window) { int idx = Windows.IndexOf(window); if (idx >= 0) { Windows.RemoveAt(idx); Windows.Add(window); } else { Windows.Add(window); } }
        ...
        public static void DrawAll() {
            ...
            for (int i = 0; i < Windows.Count; i++) {
                ...
                if (!_perfTrackingEnabled || isTaskMgr) { w.OnDraw(); continue; }
                Allocator.CurrentOwnerId = w.OwnerId;
                ulong t0 = Timer.Ticks; w.OnDraw(); ulong t1 = Timer.Ticks; ulong dt = t1 >= t0 ? t1 - t0 : 0UL;
        ...
        private static void UpdateCpuPercents() {
            ulong now = Timer.Ticks; ulong elapsed = now >= _cpuEpochTick ? now - _cpuEpochTick : 0UL; if (elapsed < 1000UL) return; if (elapsed == 0) elapsed = 1;
        ...
        public static void InputAll() {
            for (int i = 0; i < Windows.Count; i++) {
                var w = Windows[i]; if (!w.Visible) continue;
                if (_perfTrackingEnabled && !(w is guideXOS.DefaultApps.TaskManager)) Allocator.CurrentOwnerId = w.OwnerId;
                w.OnInput();
                if (_perfTrackingEnabled && !(w is guideXOS.DefaultApps.TaskManager)) Allocator.CurrentOwnerId = 0;
            }
        }
        ...
        public static bool MouseHandled { get => _mouseHandled; set => _mouseHandled = value; }
        /// <summary>
        /// Expose last CPU% for a window id
        /// </summary>
        public static int GetWindowCpuPct(int ownerId) { return _perfTrackingEnabled && _cpuPct.ContainsKey(ownerId) ? _cpuPct[ownerId] : 0; }
    }
}